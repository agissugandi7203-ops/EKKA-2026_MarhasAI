...
import React, { Children, cloneElement, isValidElement, useEffect, useMemo, useRef, forwardRef, type ReactNode } from "react";
...
const placeNow = (el: HTMLElement | null, slot: SlotPosition, skew: number) => {
  if (!el) return;
  gsap.set(el, {
    x: slot.x,
    y: slot.y,
...
export default function CardSwap({
  ...
  const rendered = childArr.map((child, i) =>
    isValidElement(child)
      ? cloneElement(child as React.ReactElement<{ ref?: React.Ref<HTMLDivElement>; style?: React.CSSProperties; onClick?: (e: React.MouseEvent) => void }>, {
          key: i,
          ref: refs[i],
          style: { width, height, ...((child.props as Record<string, unknown>).style as React.CSSProperties ?? {}) },
